...
class InventoryDatabase(context: Context?) : SQLiteOpenHelper(context, DATABASE_NAME, null, VERSION) {
    ...
    fun getTop10Items(databaseReference: DatabaseReference): Flow<List<Item>> = callbackFlow {
        val listener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                // Sort the items by count in descending order (assuming count is a comparable type)
                val top10Items = allItems.sortedByDescending { it.count?.toIntOrNull() ?: 0 }.take(10)

                // Emit the top 10 items to the flow
                trySend(top10Items)
            }
